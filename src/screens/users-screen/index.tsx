...
export default function UsersScreen() {
  ...
  const { users, loading, error, lastFetch } = useAppSelector(
    (state) => state.users,
  );
  ...
  const showNetworkStatus = useCallback(() => {
    ...
    Alert.alert("Network Status", `Status: ${statusText}${cacheInfo}`, [
      { text: "OK" },
    ]);
  }, [isOnline, lastFetch]);
    ...
    () => (
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>
          {users.length} users • {isOnline ? "Online" : "Offline"}
        </Text>
        {lastFetch && (
        ...
        <View style={styles.offlineBanner}>
          ...
          <Text style={styles.offlineBannerText}>
            Data is automatically cached for offline viewing
          </Text>
        </View>
      </View>
    ),
    ...
    () => (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyTitle}>
          {isOnline ? "Loading users..." : "No cached users"}
        </Text>
        <Text style={styles.emptySubtitle}>
          {isOnline
            ? "Fetching data from the server"
            : "Connect to internet to fetch user data"}
        </Text>
        {!loading && (
          <TouchableOpacity style={styles.retryButton} onPress={handleRefresh}>
            <Text style={styles.retryButtonText}>
              {isOnline ? "Retry" : "Try when online"}
            </Text>
          </TouchableOpacity>
        )}
      </View>
    ...
    <SafeAreaView style={styles.container}>
      <FlatList
        ...
        contentContainerStyle={[
          styles.listContent,
          users.length === 0 && styles.emptyContentContainer,
        ]}
        ...
        <View style={styles.loadingOverlay}>
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>
              {isOnline ? "Fetching users..." : "Loading cached data..."}
            </Text>
          </View>
        </View>
      )}
